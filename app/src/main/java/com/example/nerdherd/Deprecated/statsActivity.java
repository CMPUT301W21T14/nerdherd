...
public class statsActivity extends AppCompatActivity {
    ...
    private ArrayList<Experiment_Deprecated> savedList;
    private ArrayList<Experiment_Deprecated> showList;
...
//    private ArrayList<Integer> trials_1;
    ...
    private Experiment_Deprecated targetexp;
...
//        recyclerView = findViewById(R.id.subscription_experiment_recyclerView);
        ...
        savedList = new ArrayList<Experiment_Deprecated>();
        showList = new ArrayList<Experiment_Deprecated>();
        fireStoreController = new FireStoreController();
        targetexp = GlobalVariable.experimentDeprecatedArrayList.get(GlobalVariable.indexForExperimentView);
        ...
        fireStoreController.experimentReader(savedList, new FireStoreController.FireStoreExperimentReadCallback() {
            @Override
            public void onCallback(ArrayList<Experiment_Deprecated> experimentDeprecateds) {

                for (Experiment_Deprecated allExperimentDeprecated : experimentDeprecateds){
                    if (allExperimentDeprecated.getTitle().contains(targetexp.getTitle())){
                        showList.add(allExperimentDeprecated);
                    }
                }
...
//    }
    //need to convert to integer array
    public ArrayList<Integer> CountConvert(ArrayList<Experiment_Deprecated> showList){
        ...
        if (targetexp.getTrials().size() == 0){
            ...
            for (int counter = 0; counter < targetexp.getTrials().size();counter++){
//            if showList.get(counter).
                Integer counted_val = ((CountTrialDeprecated)showList.get(0).getTrials().get(counter)).getTotalCount();
                trials.add(counted_val);
            }
        }
        return trials;
    ...
    //need to convert to integer array
    public ArrayList<Integer> BinomialConvert(ArrayList<Experiment_Deprecated> showList) {
...
//                Log.d("values test", String.valueOf(((BinomialTrial)showList.get(counter).getTrials().get(0)).getSuccess()));
                Integer success_trial = ((BinomialTrialDeprecated) showList.get(0).getTrials().get(counter)).getSuccess();
                Integer failure_trial = ((BinomialTrialDeprecated) showList.get(0).getTrials().get(counter)).getFailure();
...
//        assuming values given are integers
    ...
    public void setTarget(Experiment_Deprecated exp) {
        targetexp = exp;
    }
